@echo off
REM ====================================================================
REM   REALTRAFFIC LOGIN FIX - PERMANENT ONE-CLICK SOLUTION
REM   ─────────────────────────────────────────────────
REM   1. Saari conflicting backend containers hata deta hai
REM   2. Force resets admin credentials to admin@realtraffic.local / admin123
...
title RealTraffic LOGIN FIX - Permanent Solution
color 0A
cls

echo.
echo  ============================================================
echo    REALTRAFFIC LOGIN FIX - Permanent Solution
echo  ============================================================
echo.
...
REM ───── Find ALL realtraffic-backend containers (might be multiple) ─────
echo  [2/9] Saare RealTraffic backend containers dhoondh raha hun...
set "CONTAINER_COUNT=0"
for /f "delims=" %%c in ('docker ps -a --filter "name=realtraffic-backend" --format "{{.Names}}" 2^>nul') do (
...
if not defined WORKDIR (
    if exist "F:\online\real flow\real flow amna\realtraffic-amna-main\realtraffic-amna-main\docker-compose.yml" (
        set "WORKDIR=F:\online\real flow\real flow amna\realtraffic-amna-main\realtraffic-amna-main"
    )
)
...
REM ───── KILL ALL realtraffic containers (clean slate) ─────
echo  [3/9] Saari purani RealTraffic containers hata raha hun...
...
echo  [4/9] .env file reset to admin@realtraffic.local / admin123
...
>> "!ENV_FILE!" echo ADMIN_EMAIL=admin@realtraffic.local
...
for /f "delims=" %%e in ('docker exec realtraffic-backend env 2^>nul ^| findstr /B /C:"ADMIN_EMAIL=" /C:"ADMIN_PASSWORD="') do (
    echo        %%e
)
echo.
...
if "!LOCAL_OK!"=="1" if "!PUBLIC_OK!"=="0" (
    ...
    docker ps -a --filter "name=realtraffic-cloudflared" --format "       {{.Names}}: {{.Status}}"
    echo.
