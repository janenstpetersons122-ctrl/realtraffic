# ╔══════════════════════════════════════════════════════════════════╗
# ║                                                                  ║
# ║            REALTRAFFIC — ONE-CLICK DEPLOY / UPGRADE                 ║
# ║                                                                  ║
...
# ║         amna00661226-create/realtraffic-amna/main/                   ║
# ║         REALTRAFFIC-DEPLOY.ps1 | iex                                 ║
# ║                                                                   ║
# ║  OR (after first install):                                        ║
# ║   cd C:\realtraffic                                                  ║
# ║   .\REALTRAFFIC-DEPLOY.ps1                                           ║
# ║                                                                   ║
# ╚══════════════════════════════════════════════════════════════════╝
...
[CmdletBinding()]
param(
    [string]$InstallPath  = "C:\realtraffic",
    [string]$RepoUrl      = "https://github.com/amna00661226-create/realtraffic-amna.git",
    [string]$Branch       = "main",
    [switch]$SkipBackup,
    [switch]$Force
)
...
H1 "RealTraffic Deploy — $((Get-Date).ToString('yyyy-MM-dd HH:mm'))"
...
if ($IsExisting) {
    ...
    git stash push -u -m "auto-stash by REALTRAFFIC-DEPLOY $(Get-Date -Format yyyyMMdd-HHmmss)" 2>&1 | Out-Null
    git fetch origin
    git checkout $Branch
    ...
    if (Test-Path $InstallPath) {
        if (-not $Force) {
            Err "$InstallPath exists but is not a RealTraffic install. Use -Force to overwrite."
            exit 1
        }
...
if (-not (Test-Path $envFile)) {
    ...
    Write-Host "    │ Admin Email    : admin@realtraffic.local                │"  -ForegroundColor Yellow
...
Write-Host "    .\REALTRAFFIC-UPDATE.bat      # quick update next time"
Write-Host "    .\REALTRAFFIC-STOP.bat        # stop everything"
Write-Host ""
Write-Host "  CPI Worker (when phones arrive):" -ForegroundColor Cyan
Write-Host "    .\deployment\cpi\REALTRAFFIC-CPI-SETUP.ps1"
Write-Host ""

# Mark this PowerShell session as deployed
$global:REALTRAFFIC_DEPLOYED = $true
