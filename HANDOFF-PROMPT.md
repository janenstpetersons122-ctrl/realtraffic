# RealTraffic Project — Collaboration Handoff Prompt
...
```
PROJECT HANDOFF — RealTraffic CPI / Real-User-Traffic Automation Platform
...
RealTraffic is a full-stack web app + Python automation suite for two main use cases:
...
- Frontend: React (CRA) — production deployed at https://realtraffic.online
- Backend: FastAPI + MongoDB
- Backend hosted in Docker on the owner's Windows PC (NOT on Emergent or Vercel)
- Public access via Cloudflare Tunnel: api.realtraffic.online → realtraffic-backend:8001
- Domain DNS: realtraffic.online (Cloudflare proxied)

Standalone (Windows PC):
- Docker Compose: realtraffic-mongo + realtraffic-backend + realtraffic-cloudflared
- Master deploy script: RealTraffic-AUTO.bat (auto git pull + container rebuild)
- Tunnel setup helper: FIX-TUNNEL.bat
- Diagnostic helper: DIAGNOSE.bat
- CPI worker: /realtraffic-cpi-worker/ — separate Python project for ADB control
...
✅ Cloudflare Tunnel chal raha hai — public URL realtraffic.online live
...
✅ One-click Windows deploy script (RealTraffic-AUTO.bat)
...
LOCAL PRODUCTION (Windows PC, https://realtraffic.online):
- Admin email: us9661626@gmail.com
- Admin password: (owner will provide separately — DO NOT share publicly)
- Default seed admin (also exists): admin@realtraffic.local / admin123

CLOUD PREVIEW (Emergent k8s):
- Admin: admin@realtraffic.local / admin123
...
1. **https://github.com/<APKA_USERNAME>/<REPO_NAME>** kholo (jahan aapka realtraffic code hai)
