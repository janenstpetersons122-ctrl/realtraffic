# RealTraffic CPI Worker

Polls the RealTraffic backend (`api.realtraffic.online`) for queued install attempts
...
Runs on the user's home PC alongside the existing `realtraffic-backend` Docker stack.
...
2. Copy `config.example.yaml` → `config.yaml` and edit:
   - `api.base_url` = your `https://api.realtraffic.online`
   - `api.token`    = JWT from RealTraffic web (Profile → Copy Token)
