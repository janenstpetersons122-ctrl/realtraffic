"""
RealTraffic CPI — Local iOS Proxy Gateway
...
Workaround used here (zero touch after one-time setup):
  ...
  • This mitmproxy add-on then forwards each iPhone request through a
    ...
    populated live by the RealTraffic CPI Worker (orchestrator) per-install.
...
The worker writes to:
  /tmp/realtraffic-cpi-ios-proxy-pool.json     (Linux/macOS)
  C:\\realtraffic\\realtraffic-cpi-worker\\ios-proxy-pool.json  (Windows)
...
POOL_PATHS = [
    Path("C:/realtraffic/realtraffic-cpi-worker/ios-proxy-pool.json"),
    Path("/tmp/realtraffic-cpi-ios-proxy-pool.json"),
    Path(os.path.expanduser("~/.realtraffic-cpi-ios-proxy-pool.json")),
]
