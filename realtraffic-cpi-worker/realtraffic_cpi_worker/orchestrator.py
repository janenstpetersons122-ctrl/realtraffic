"""Orchestrator — top-level loop that:
  ...
  • Registers them with the RealTraffic backend
  • Sends heartbeats
...
class Orchestrator:
    ...
    async def run(self):
        logger.info(f"RealTraffic CPI Worker starting → backend={self.cfg.api.base_url}")
        try:
...
async def run_doctor(cfg: Config) -> int:
    print("RealTraffic CPI Worker — Doctor\n" + "─" * 40)
