RealTraffic CPI Worker — Deployment scripts (Windows 11)
═══════════════════════════════════════════════════════
...
REALTRAFFIC-CPI-WORKER-START.bat
    Starts the worker in foreground. Press Ctrl+C to stop.

REALTRAFFIC-CPI-WORKER-STOP.bat
    Kills any running worker process.

REALTRAFFIC-CPI-DOCTOR.ps1
...
───────────────────
  ...
  1. SETUP        →  Run REALTRAFFIC-CPI-SETUP.ps1 once (Administrator)
  2. CONFIGURE    →  Edit ..\..\realtraffic-cpi-worker\config.yaml
                     • api.token = your RealTraffic JWT
  3. CONNECT      →  Plug in Android phone (USB debugging ON)
                     Plug in iPhone (Trust Computer when prompted)
  4. VERIFY       →  REALTRAFFIC-CPI-DOCTOR.ps1
  5. RUN          →  REALTRAFFIC-CPI-WORKER-START.bat
...
• "iPhone not appearing in tidevice3 list"
  ...
  → 3) Re-run REALTRAFFIC-CPI-DOCTOR.ps1.

• Worker logs into wrong account / "auth failed"
  → JWT expired. On https://realtraffic.online, login again, copy fresh token,
    paste into config.yaml under api.token. Restart worker.

• Devices show "offline" in RealTraffic web UI
  → Worker is not running, or its IP cannot reach api.realtraffic.online.
  → Check INSTALL-WORKER-AS-SERVICE service status: `nssm status RealTrafficCPIWorker`
