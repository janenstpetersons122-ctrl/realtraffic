@echo off
REM ======================================================================
REM   REALTRAFFIC CPI - PROXY BRIDGE DIAGNOSTIC & AUTO-FIX
...
title RealTraffic CPI - Proxy Bridge Diagnostic
color 0E
cls

set "ROOT=F:\online\real flow\lenovo real flow\lenovo-realtraffic-main\lenovo-realtraffic-main\realtraffic-cpi-worker"
...
if not exist "%ROOT%\realtraffic_cpi_worker\proxy_bridge.py" (
...
findstr /C:"get_bridge" "%ROOT%\realtraffic_cpi_worker\android_engine.py" >nul
if errorlevel 1 (
...
from realtraffic_cpi_worker.proxy_bridge import ProxyBridge; \
async def main(): \
...
powershell -NoProfile -Command "Get-NetFirewallRule -DisplayName 'RealTraffic CPI Proxy Bridge' -ErrorAction SilentlyContinue | Remove-NetFirewallRule" 2>nul
powershell -NoProfile -Command "New-NetFirewallRule -DisplayName 'RealTraffic CPI Proxy Bridge' -Direction Inbound -LocalPort 8788 -Protocol TCP -Action Allow -Profile Any" >nul 2>&1
