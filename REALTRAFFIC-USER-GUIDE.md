# 📘 RealTraffic — Complete User Guide

**Version**: 1.0 (May 2026)
**Repo**: https://github.com/lenovogen03/lenovo-realtraffic
**Production**: https://realtraffic.online
**Backend**: https://api.realtraffic.online
...
### 🏁 Getting Started
1. [RealTraffic kya hai?](#1-realtraffic-kya-hai)
...
### 🔄 Updates & Deployment
47. [How to Update Code (REALTRAFFIC-FORCE-SYNC)](#47-how-to-update-code-realtraffic-force-sync)
...
## 1. RealTraffic kya hai?

RealTraffic ek **multi-feature traffic & lead automation platform** hai jo 4 main kaam karta hai:
...
- App URL: https://realtraffic.online ya aapka custom domain
...
1. **Repo clone karen**:
   ```powershell
   cd F:\online
   git clone https://github.com/lenovogen03/lenovo-realtraffic.git
   cd lenovo-realtraffic
   ```
...
2. **`.env` file banayen** `backend/` folder mein:
   ...
   APP_URL=https://realtraffic.online
   PUBLIC_BASE_URL=https://api.realtraffic.online
   TUNNEL_TOKEN=your-cloudflare-tunnel-token
   ```

3. **Cloudflare Tunnel setup karen**:
   - https://one.dash.cloudflare.com/ → Networks → Tunnels → Create
   - Naam: `realtraffic-tunnel`
   - Token copy karen → `.env` ke `TUNNEL_TOKEN` mein paste
   - Domain configure karen: `api.realtraffic.online → http://backend:8001`

4. **Service Account JSON download karen** (Google Sheets ke liye — optional but recommended):
   - https://console.cloud.google.com → New Project → "RealTraffic Sheets"
...
6. **`FRESH-DEPLOY.bat` ya `REALTRAFFIC-FORCE-SYNC.bat` chalayen** (project folder mein double-click)

7. **Health check**: Browser mein khol ke verify karen:
   - `https://api.realtraffic.online/health` → `mongo_connected: true` aana chahiye

8. **Admin login** karen:
   - URL: https://realtraffic.online/admin
...
1. Project folder mein jayen:
   ```powershell
   cd F:\online\lenovo-realtraffic
   ```

2. **`REALTRAFFIC-FORCE-SYNC.bat`** chalayen (double-click ya CMD se):
   ```cmd
   REALTRAFFIC-FORCE-SYNC.bat
   ```
...
|---|---|---|
| **Stop all** | `REALTRAFFIC-STOP.bat` | `docker compose down` |
| **Start all** | `REALTRAFFIC-AUTO.bat` | `docker compose up -d` |
| **Restart backend only** | — | `docker compose restart backend` |
| **Rebuild backend** | `REALTRAFFIC-UPDATE.bat` | `docker compose up -d --build backend` |
| **Force fresh** | `REALTRAFFIC-FORCE-SYNC.bat` | (See script) |
| **View logs** | `REALTRAFFIC-LOGS.bat` | `docker compose logs -f backend` |
...
**URL**: https://realtraffic.online/login
...
**First-time register**: https://realtraffic.online/register
...
1. Email: `admin@realtraffic.local` (ya jo `ADMIN_EMAIL` `.env` mein set hai)
...
3. Email check karen — RealTraffic se 1-hour-valid reset link aayega
...
1. **URL**: https://realtraffic.online/login (same login page)
...
**Step 3: RealTraffic Admin Panel mein paste karen**
...
6. **Sender name**: `RealTraffic Alerts` (ya jo dikhana ho inbox mein "From" pe)
...
4. Inbox check karen — `RealTraffic Alerts` se email aayegi

### **Option B — Resend (free 3,000/month, custom domains)**

1. https://resend.com/signup → account banayen
2. https://resend.com/api-keys → "Create API Key" → naam `RealTraffic`, permission `Sending access`
...
| Field | Description |
|---|---|
| **App Name** | "RealTraffic" → kuch aur set kar sakte hain |
...
### **Faiday**:
- Sheet kabhi bhi edit karen — RealTraffic auto-pickup karega next job pe
...
4. **RealTraffic mein paste karen**:
...
1. Google Sheets mein **ek master spreadsheet** banayen: e.g. "RealTraffic Data Hub"
...
3. Name: `realtraffic-sheets` → Create
...
3. Name: `realtraffic-bot` → Create → Done

### **Step 4: JSON key download**
1. Service account list mein `realtraffic-bot@xxx.iam.gserviceaccount.com` pe click
...
1. Apni Windows PC pe project folder kholen:
   ```
   F:\online\real flow\lenovo real flow\lenovo-realtraffic-main\lenovo-realtraffic-main\backend\
   ```
...
### **Step 6: Restart**
- `REALTRAFFIC-FORCE-SYNC.bat` chalayen ya `docker compose restart backend`
...
### Generated short link milega: `https://realtraffic.online/r/abc123`
...
Postback flow:
1. Network conversion fire karta hai → RealTraffic ka postback URL hit karta hai
2. RealTraffic conversion record karta hai
...
```
https://api.realtraffic.online/api/postback/{your-postback-token}?click_id={click_id}&payout={payout}&status=approved
```
...
### **Test karen**:
```bash
curl "https://api.realtraffic.online/api/postback/your-token?click_id=test123&payout=2.50&status=approved"
```
...
4. ADB se phone detect hote hi RealTraffic ko notify karega
5. RealTraffic → CPI page → connected devices list
...
### **Manual backup**:
```cmd
docker exec realtraffic-mongo mongodump --out=/data/db/backup-2026-05-04
docker cp realtraffic-mongo:/data/db/backup-2026-05-04 F:\realtraffic-backups\
```

### **Automated** (recommended):
1. Task Scheduler kholen (Windows)
2. New task: daily 3 AM
3. Action: `docker exec realtraffic-mongo mongodump --out=/data/db/backup-%date%`
...
```cmd
docker cp F:\realtraffic-backups\backup-2026-05-04 realtraffic-mongo:/data/db/restore
docker exec realtraffic-mongo mongorestore --drop /data/db/restore
docker compose restart backend
```
...
Easy way: `REALTRAFFIC-LOGS.bat` double-click karen.
...
4. **Settings** → **Environment Variables**: `REACT_APP_BACKEND_URL=https://api.realtraffic.online` set hai?
...
## 47. How to Update Code (REALTRAFFIC-FORCE-SYNC)

Already covered in [Section 4](#4-daily-update--resync). Quick recap:

1. Project folder mein **`REALTRAFFIC-FORCE-SYNC.bat`** double-click
2. 3-5 min wait
3. Verify `https://api.realtraffic.online/health`
...
```cmd
docker exec realtraffic-mongo mongodump --out=/data/db/pre-update-backup
```

Phir `REALTRAFFIC-FORCE-SYNC.bat` chalayen.
...
│   │   └── ...
│   └── package.json
├── realtraffic-cpi-worker/          # Native Python CPI worker (Windows)
...
| Script | Use case |
|---|---|
| **REALTRAFFIC-AUTO.bat** | First-time auto setup (full pipeline) |
| **REALTRAFFIC-FORCE-SYNC.bat** ⭐ | Pull latest code + rebuild + restart (use after every update) |
| **REALTRAFFIC-UPDATE.bat** | Light update (no `--no-cache` rebuild) |
| **REALTRAFFIC-STOP.bat** | Stop all containers |
| **REALTRAFFIC-LOGS.bat** | Tail backend logs in real-time |
| **REALTRAFFIC-DOCTOR.bat** | Health diagnostics |
| **REALTRAFFIC-LOGIN-FIX.bat** | Re-seed admin if login issues |
| **REALTRAFFIC-DEPLOY.ps1** | PowerShell version of deploy (with more options) |
...
| Account | Default |
|---|---|
| **Admin email** | `admin@realtraffic.local` (in `.env` `ADMIN_EMAIL`) |
...
1. **Stop**: `REALTRAFFIC-STOP.bat`
2. **Backup MongoDB volume**:
   ```cmd
   docker run --rm -v realtraffic_mongo-data:/data -v F:\backups:/backup ubuntu tar czf /backup/mongo-emergency.tar.gz /data
   ```
3. **Fresh clone karen**:
   ```cmd
   cd F:\online
   git clone https://github.com/lenovogen03/lenovo-realtraffic.git realtraffic-fresh
   cd realtraffic-fresh
   ```
...
6. **Restore Mongo** (agar zarurat ho):
   ...
   docker run --rm -v realtraffic-fresh_mongo-data:/data -v F:\backups:/backup ubuntu tar xzf /backup/mongo-emergency.tar.gz -C /
...
```
┌─────────────────────────────────────────────────────┐
│  RealTraffic Daily Operations                          │
├─────────────────────────────────────────────────────┤
│                                                     │
│  Update code:    REALTRAFFIC-FORCE-SYNC.bat            │
│  Stop server:    REALTRAFFIC-STOP.bat                  │
│  Start server:   REALTRAFFIC-AUTO.bat                  │
│  View logs:      REALTRAFFIC-LOGS.bat                  │
│  Health check:   https://api.realtraffic.online/health │
│  Admin login:    https://realtraffic.online/admin       │
│  User login:     https://realtraffic.online/login       │
│  Repo:           github.com/lenovogen03/lenovo-realtraffic │
│                                                     │
└─────────────────────────────────────────────────────┘
```
...
- **GitHub Issues**: https://github.com/lenovogen03/lenovo-realtraffic/issues
- **Docs**: This file (`REALTRAFFIC-USER-GUIDE.md`)
- **Owner**: lenovogen03
