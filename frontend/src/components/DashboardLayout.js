...
import { LayoutDashboard, Link2, MousePointerClick, DollarSign, Server, Menu, LogOut, User, Settings, TrendingUp, Upload, Mail, Filter, Smartphone, Search, ClipboardCheck, Fingerprint, Package, Apple, Cpu, Briefcase, ChevronDown, ChevronRight, Link as LinkIcon } from "lucide-react";
        ...
        <div className="p-6 flex items-center justify-between" style={{ borderBottom: '1px solid var(--brand-border)' }}>
          {sidebarOpen && (
            branding.logo_url ? (
              ...
              <h1 className="text-xl font-bold" style={{ color: 'var(--brand-text)' }} data-testid="app-logo">{branding.app_name || "RealTraffic"}</h1>
            )
          )}
          <Button
            variant="ghost"
