...
export default function RealUserTrafficPage() {
  ...
  // Fast Mode — block images, fonts, media, analytics on landing pages for
  // 3-5x faster visits + 40% less Chromium RAM. Thank-you page auto-unblocks
  // so the proof screenshot stays intact. Recommended ON for 16GB RAM PCs.
  ...
  const [postSubmitWait, setPostSubmitWait] = useState(3);
  ...
  const onStart = async () => {
    ...
    try {
      ...
      let autoDataId = "";
      if (formFillEnabled && dataSource === "excel" && file && !selectedUploadDataId) {
        autoDataId = await preUpload("Excel data file",
      ...
      // Proxies (any non-empty paste) — skip if using stored or picked a batch
      let autoProxyId = "";
      if (!useStoredProxies && !selectedUploadProxyId && proxies && proxies.trim()) {
        autoProxyId = await preUpload("Proxies",
      ...
      // User-Agents (any non-empty paste)
      let autoUaId = "";
      if (!selectedUploadUaId && selectedUploadUaIds.length === 0 && userAgents && userAgents.trim()) {
        autoUaId = await preUpload("User-Agents",
      ...
      // Custom Automation JSON paste (keep as ID when possible)
      let autoAjId = "";
      if (formFillEnabled && useCustomJson && !selectedUploadAjId && automationJson.trim()) {
        autoAjId = await preUpload("Automation JSON",
      ...
      // Target screenshot file (pHash computed server-side at pre-upload
      // time so the job-create request no longer has to process an image)
      let autoTargetScreenshotId = "";
      if (targetScreenshotFile) {
        autoTargetScreenshotId = await preUpload("Target screenshot",
      ...
      // Resilient submit: bounded timeout + ONE silent retry on transient
      // network abort. Cloudflare-tunnel idle-timeouts are usually ~100s;
      // we cap at 110s and retry once with a fresh AbortController. After
      // that we fall through to the auto-recovery polling block below.
      ...
      try {
        ...
        // One quick retry — usually the second attempt succeeds because
        // the backend has just finished any cold-start gsheet fetches
        // and cached `last_synced_at`, so the next call returns fast.
      ...
      // even after the silent retry, the backend may STILL have already
      // accepted the job (the tunnel can drop the response while the
      // server keeps processing). Poll the past-jobs list for up to 60s
      // and auto-recover if a new job appeared for this user.
      ...
      let friendly = raw;
      if (isNetworkAbort) {
        friendly = "Network aborted — likely reasons: (1) Excel/Gsheet too large and upload timed out, " +
                   "(2) backend container briefly unreachable, (3) home PC temporarily slow. " +
                   "Steps: refresh Past Jobs list — the job may still appear. " +
                   "If not, try reducing the lead-file size, or pre-upload files via 'Uploaded Things' page and re-run.";
      }
      ...
      <Card className="bg-zinc-900 border-zinc-800">
        ...
        <CardContent className="space-y-5">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              <Label className="text-zinc-300 text-sm">Concurrency (1–15)</Label>
              <Input
                ...
                max={15}
                value={concurrency}
                onChange={(e) => setConcurrency(Math.max(1, Math.min(15, Number(e.target.value) || 1)))}
                className="mt-1 bg-zinc-800 border-zinc-700 text-zinc-100"
              />
              <p className="text-xs text-zinc-500 mt-1">
                Recommended: 3–8 for 16GB RAM PC, up to 12 max. Each concurrent visit eats ~100MB Chromium RAM.
              </p>
            </div>
          </div>

          {/* Fast Mode toggle */}
          <div className="mt-4 p-3 rounded-lg border border-amber-700/30 bg-amber-950/20">
            <label className="flex items-start gap-3 cursor-pointer" data-testid="rut-fast-mode-label">
              <input
                type="checkbox"
                data-testid="rut-fast-mode-toggle"
                checked={fastMode}
                onChange={(e) => setFastMode(e.target.checked)}
                className="mt-1 h-4 w-4 rounded border-amber-700 bg-zinc-900 text-amber-500 focus:ring-amber-500"
              />
              <div className="flex-1">
                <div className="text-sm font-medium text-amber-200">⚡ Fast Mode — block images, fonts, ads, trackers</div>
                <div className="text-xs text-amber-200/70 mt-0.5">
                  Drops per-visit time 3-5x and Chromium RAM ~40%. Thank-you screenshot auto-unblocks so proof stays intact.
                  Recommended ON if you're on a 16GB RAM PC or running concurrency ≥ 8.
                </div>
              </div>
            </label>
          </div>
