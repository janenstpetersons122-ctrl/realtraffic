...
title RealTraffic - Admin Password Reset
color 0E

REM ====================================================================
REM   RealTraffic ONE-CLICK ADMIN PASSWORD RESET
REM   ──────────────────────────────────────
...
echo.
echo  ============================================================
echo    RealTraffic - ADMIN PASSWORD RESET
echo  ============================================================
echo.

REM Auto-detect: agar script kisi RealTraffic root mein hai, wahin chalega
...
powershell -NoProfile -Command ^
    ...
    "foreach($l in $lines) {" ^
        "if ($l -match '^\s*ADMIN_EMAIL\s*=') { $out += 'ADMIN_EMAIL=admin@realtraffic.local'; $hasEmail=$true }" ^
        "elseif ($l -match '^\s*ADMIN_PASSWORD\s*=') { $out += 'ADMIN_PASSWORD=admin123'; $hasPass=$true }" ^
        "else { $out += $l }" ^
    "};" ^
    "if (-not $hasEmail) { $out += 'ADMIN_EMAIL=admin@realtraffic.local' };" ^
...
curl -s -X POST "http://127.0.0.1:8001/api/admin/login" ^
    ...
    -d "{\"email\":\"admin@realtraffic.local\",\"password\":\"admin123\"}" > "%TEMP%\admin_test.json" 2>nul
...
echo  ============================================================
echo.
echo    Admin Email   : admin@realtraffic.local
echo    Admin Password: admin123
echo.
echo    Ab browser mein:
echo      1. Apna RealTraffic URL kholein
