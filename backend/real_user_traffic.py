...
# In-memory cache: proxy_server_key -> (geo_result_dict, inserted_at_ts).
# Same proxy used across multiple visits in the SAME job → 0-cost geo
# lookup the 2nd time. Saves 1-5s per repeat visit. Cache lives only
# in-process so it auto-clears when the backend restarts.
_GEO_CACHE: Dict[str, Tuple[Dict[str, Any], float]] = {}
_GEO_CACHE_TTL_SEC = 900  # 15 minutes — residential proxies rotate exit IPs slowly

def _geo_cache_get(key: str) -> Optional[Dict[str, Any]]:
    rec = _GEO_CACHE.get(key)
    if not rec:
        return None
    data, ts = rec
    if (time.time() - ts) > _GEO_CACHE_TTL_SEC:
        _GEO_CACHE.pop(key, None)
        return None
    # Shallow copy so callers mutating result don't corrupt the cache
    return dict(data)

def _geo_cache_set(key: str, value: Dict[str, Any]) -> None:
    try:
        _GEO_CACHE[key] = (dict(value), time.time())
    except Exception:
        pass
...
async def _probe_proxy_geo(proxy: Dict[str, Any], ua: str) -> Dict[str, Any]:
    ...
    try:
        # Cache lookup — same proxy within the job (or within 15 min across
        # jobs) reuses the previous geo result. Drops 1-5s off every repeat
        # visit. Key = raw proxy server URI (without credentials).
        cache_key = proxy.get("server") or ""
        cached = _geo_cache_get(cache_key)
        if cached:
            return cached

        # Residential proxies can take 10-15s to route; 12s is plenty and
        # is below Cloudflare Tunnel's 100s budget even for 2 retries.
        # Retry up to 2 times (was 3) — diminishing returns past 2.
        timeout_cfg = httpx.Timeout(12.0, connect=6.0)
        ok = False
        ...
        if ok:
            ...
            result["ok"] = True
            # Cache the successful probe result for reuse within 15 min
            _geo_cache_set(cache_key, result)
    except Exception as e:
...
async def run_real_user_traffic_job(
    ...
    # Drops per-page load time 30-70% and Chromium RAM usage ~40%.
    # The final thank-you page is auto-unblocked so screenshot quality
    # stays intact.
    ...
    async def _live_remove_proxy(raw: str):
        ...
        try:
            client = db.client
            user_db = client[f"realtraffic_user_{user_db_truncated}"]
        ...
        async with _data_file_lock:
            try:
                ...
                if gsheet_url:
                    ...
                    try:
                        user_doc = await client["realtraffic"]["users"].find_one(
                            {"id": engine_user_id},
    ...
    async def process_one(i: int, shared_browser: Browser):
        ...
        try:
            ...
            if True:
                ...
                # Fast Mode — block heavy resources on non-thank-you pages.
                # We abort image/font/media/tracker requests; the browser
                # still renders structure + clickable elements, so form-fill
                # still works. Savings: ~30-70% per-visit time + ~40% RAM.
                _fast_mode_allowed_types = {"document", "script", "stylesheet", "xhr", "fetch", "websocket"}
                _fast_mode_blocked_hosts = (
                    "google-analytics.com", "googletagmanager.com", "doubleclick.net",
                    "facebook.net", "facebook.com/tr", "hotjar.com", "clarity.ms",
                    "analytics.tiktok.com", "bat.bing.com", "connect.facebook.net",
                    "cdn.segment.com", "amplitude.com", "mixpanel.com",
                )
                if fast_mode:
                    async def _fast_mode_route(route):
                        try:
                            req = route.request
                            rtype = req.resource_type
                            url = req.url
                            if rtype not in _fast_mode_allowed_types:
                                return await route.abort()
                            if any(h in url for h in _fast_mode_blocked_hosts):
                                return await route.abort()
                            ...
                            try:
                                ...
                                pass
                    try:
                        await page.route("**/*", _fast_mode_route)
                    except Exception:
                        pass
                ...
                # the DOM. Reduced from 20s → 8s — heavy offer sites almost
                # never reach true networkidle within 20s anyway, so the
                # extra wait was mostly dead time.
                try:
                        ...
                        if skip_captcha and await _page_has_captcha(page):
                            ...
                            async def _pre_submit_cb(label: str) -> None:
                                try:
                                    ...
                                    # full_page=False is ~3-5x faster than full_page=True
                                    # and shows the visible viewport which is all we need
                                    # to debug what the user sees before a submit click.
                                    await page.screenshot(
                                        path=str(shot_path_ps), full_page=False,
                                    )
                                    _pre_submit_shots.append(shot_name)
                                    push_live_step(
                                        job_id, i + 1, "pre_submit", "info",
                                        f"Captured pre-submit screenshot ({label})",
                ...
                try:
                    ...
                    push_live_step(
                        ...
                        f"Captured post-submit screenshot ({post_submit_wait}s after submit) → {entry.get('final_url', '')[:90]}",
                ...
                if entry.get("thank_you_reached"):
                    ...
                    try:
                        # Fast Mode is OFF for the final screenshot: we
                        # unblock the route so images/fonts render properly
                        # in the thank-you proof capture. No-op if Fast
                        # Mode was never enabled.
                        if fast_mode:
                            try:
                                await page.unroute("**/*")
                                # Give the page a chance to lazy-load its
                                # images now that they're unblocked.
                                try:
                        ...
                        # Reduced from 12s → 6s (ample for 95% of pages).
                        try:
                        ...
                        # Reduced from 8×1s to 4×0.5s for quicker start.
                        ...
                        # Tiny extra settle so animations finish (reduced 1.2s → 0.5s)
                        await asyncio.sleep(0.5)
...
async def _log_click_for_link(entry: Dict[str, Any], job_info: Dict[str, Any], main_db):
    ...
    try:
        ...
        #     f"realtraffic_user_{user_id.replace('-', '_')[:20]}"
        ...
        db_name = f"realtraffic_user_{owner_id.replace('-', '_')[:20]}"
...
# also counts). Do NOT add the user's OWN domain (realtraffic.online) or
