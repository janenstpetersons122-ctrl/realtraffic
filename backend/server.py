...
def get_user_db(user_id: str):
    ...
    db_name = f"realtraffic_user_{user_id.replace('-', '_')[:20]}"
...
# Each user has their own MongoDB database (`realtraffic_user_<id>`). The
...
async def get_all_click_ips_from_entire_database(force_refresh=False):
    ...
    try:
        ...
        user_dbs = [name for name in db_names if name.startswith("realtraffic_user_")]
...
async def debug_search_ip(ip: str):
    ...
    try:
        ...
        user_databases = [name for name in all_db_names if name.startswith("realtraffic_user_")]
...
# Admin configuration
ADMIN_EMAIL = os.environ.get("ADMIN_EMAIL", "admin@realtraffic.local")
...
ADMIN_CONTACT_EMAIL = "admin@realtraffic.local"
...
async def send_password_reset_email(to_email: str, user_name: str, reset_token: str, user_type: str = "user"):
    ...
    app_name = branding.get("app_name", "RealTraffic")
...
DEFAULT_BRANDING = {
    ...
    "footer_text": "© 2026 RealTraffic. All rights reserved.",
    "sidebar_style": "dark",
...
async def admin_system_check(admin: dict = Depends(get_current_admin)):
    ...
    try:
        ...
        per_user_dbs = [d for d in all_dbs if d.startswith("realtraffic_user_")]
...
async def admin_get_email_settings(admin: dict = Depends(get_current_admin)):
    ...
    return {
        ...
        "sender_name": cfg.get("sender_name") or "RealTraffic",
...
async def admin_test_email(
    ...
    <table width="100%" cellpadding="0" cellspacing="0" style="padding:32px 16px;"><tr><td align="center">
      <table width="560" cellpadding="0" cellspacing="0" style="background:#18181B;border:1px solid #27272A;border-radius:12px;">
        <tr><td style="padding:32px 28px;">
          ...
          <p style="color:#A1A1AA;margin:0 0 18px 0;font-size:13px;">RealTraffic Admin · Test message</p>
    ...
    result = await send_alert_email(
        to_email=target,
        subject="✓ RealTraffic email settings test",
        html=html,
        db=db,
    )
...
async def delete_user(user_id: str, admin: dict = Depends(get_current_admin)):
    ...
    try:
        ...
        try:
            ...
            try:
                user_db_name = f"realtraffic_user_{user_id}"
...
async def rut_create_job(
    ...
    skip_captcha: bool = Form(True),
    post_submit_wait: int = Form(3),                  # seconds 3..15 (default reduced from 6→3)
    ...
    # Fast Mode — block images, fonts, media, analytics on landing pages.
    # Drops per-visit time 30-70% and Chromium RAM usage ~40%. Thank-you
    # page is auto-unblocked so proof screenshot quality stays intact.
    # Recommended ON for 16GB RAM PCs running concurrency ≥ 8.
    ...
    if concurrency < 1 or concurrency > 15:
        raise HTTPException(status_code=400, detail="concurrency must be 1..15 (hardware-safe cap for 16GB RAM)")
    if duration_minutes < 0 or duration_minutes > 1440:
    ...
    # 4. User agents — either paste OR from one-or-more uploaded batches.
    #
    # Multi-batch support lets the user run ONE campaign that spans several
    # device buckets at once (e.g. "iPhone UAs" + "Android UAs" + "iPad UAs").
    # The engine then round-robins across the merged pool and each UA is
    # $pulled from its ORIGINAL batch when consumed (via `ua_to_batch_map`).
    ua_batch_ids: List[str] = []
    if upload_ua_ids:
        ua_batch_ids = [b.strip() for b in upload_ua_ids.split(",") if b.strip()]
    elif upload_ua_id:
        ua_batch_ids = [upload_ua_id]
    if ua_batch_ids:
        # Parallelize all per-batch loads — when several batches are
        # gsheet-backed, sequential `await`s would add ~1-3s per sheet
        # and easily push the request over Cloudflare's idle-tunnel
        # timeout. asyncio.gather collapses them into a single round-trip
        # window so 5 batches now cost ~3s total instead of ~15s.
        ...
        for bid, batch_items in zip(ua_batch_ids, batch_results):
            ...
            for ua_line in batch_items:
                if ua_line not in ua_to_batch_map:
                    # First batch to contain this UA wins — keeps things deterministic
        ...
        if not merged_ua_lines:
            raise HTTPException(
                status_code=404,
                detail="Selected user-agent upload(s) are empty or not found",
            )
    ...
    # 7d. Target screenshot — pHash for verified-conversion check
    target_screenshot_phash: Optional[str] = None
    target_screenshot_path: Optional[str] = None
    # Preferred path: pre-uploaded screenshot (small payload, fast job-create)
    if target_screenshot_upload_id:
        pair = await _load_upload_target_screenshot(user["id"], target_screenshot_upload_id)
        if not pair:
            raise HTTPException(status_code=404, detail="Selected target-screenshot upload not found")
        src_path, target_screenshot_phash = pair
        # Copy into per-job-pending dir so the existing rename-to-per-job
        # logic below still works unchanged.
        try:
    ...
    elif target_screenshot is not None:
        try:
            ts_bytes = await target_screenshot.read()
            if ts_bytes and len(ts_bytes) >= 200:
                from screenshot_verifier import compute_phash
                target_screenshot_phash = compute_phash(ts_bytes)
                if target_screenshot_phash:
                    # Persist target screenshot to disk so it shows up in
                    # the job-detail UI and can be re-used for retries
                    from pathlib import Path
                    ...
                    raise HTTPException(
                        status_code=400,
                        detail="target_screenshot is not a valid image",
                    )
        except HTTPException:
            raise
        except Exception as e:  # noqa: BLE001
    ...
    # Validate threshold
    target_screenshot_threshold = max(0, min(30, int(target_screenshot_threshold)))
    ...
    # Move target screenshot from _pending to per-job folder
    ...
    background.add_task(
        ...
        # Target screenshot verification
        target_screenshot_phash=target_screenshot_phash or "",
        target_screenshot_threshold=target_screenshot_threshold,
        # Fast Mode — blocks images/fonts/ads on landing pages for speed.
        fast_mode=fast_mode,
    )
    return {
        "job_id": job_id,
...
def _analyze_ua(ua: str) -> dict:
    ...
    # 5. Referrer-source guess (what RealTraffic will categorise this UA as)
...
async def find_click_across_dbs(clickid: str):
    ...
    Clicks are normally written to user-specific DBs (realtraffic_user_*) by the
    ...
    try:
        ...
        for name in all_db_names:
            if not name.startswith("realtraffic_user_"):
                continue
...
async def list_uploads(
    ...
    # ── Live-sync any gsheet-backed docs in parallel (TTL guarded) ──
    ...
    if sync_gsheets:
        ...
        if stale_docs:
            await asyncio.gather(
                *[_refresh_gsheet_doc(current_user["id"], d) for d in stale_docs],
                return_exceptions=True,
            )
