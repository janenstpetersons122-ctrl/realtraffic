...
async def send_alert_email(
    ...
    sender_name = _pick("sender_name", "APP_NAME") or "RealTraffic"
    sender_email = (
...
async def maybe_send_low_stock_alert(
    *,
    ...
    try:
        await send_alert_email(target, subject, html, db=user_db.client["realtraffic"] if user_db is not None else None)
    except Exception as e:
...
# main `realtraffic` DB (collection `admin_settings`, doc id `email_config`).
