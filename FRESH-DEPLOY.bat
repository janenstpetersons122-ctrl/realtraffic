@echo off
REM ====================================================================
REM   RealTraffic FRESH DEPLOY v2 (Bulletproof)
REM   ───────────────────────────────────────
...
title RealTraffic - FRESH DEPLOY (Bulletproof)
color 0A

cls
echo.
echo  ============================================================
echo    RealTraffic - FRESH DEPLOY (Bulletproof)
echo  ============================================================
echo.
...
echo    1. Saari purani RealTraffic containers + volumes HATAYI jayein gi
echo    2. SAFETY backup banega: F:\online\real flow\realtraffic-backups\
...
set "AMNA_PATH=F:\online\real flow\real flow amna\realtraffic-amna-main\realtraffic-amna-main"
...
REM Remove containers matching realtraffic-*
...
REM Remove networks matching realtraffic
...
if defined BACKED_ANY (
    ...
    for /f "delims=" %%f in ('dir /b "!BACKUP_DIR!\volumes\*.tar.gz" 2^>nul') do (
        echo    docker stop realtraffic-mongo
        echo    docker run --rm -v realtraffic-mongo-data:/data -v "!BACKUP_DIR!\volumes:/backup" alpine sh -c "rm -rf /data/* /data/.* 2^>/dev/null; tar xzf /backup/%%f -C /data"
        echo    docker start realtraffic-mongo
        goto :NORESTORE_DONE
    )
)
:NORESTORE_DONE
