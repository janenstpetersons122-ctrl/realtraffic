# RealTraffic CPI Module — Urdu Setup Guide

> **Yeh guide aap ke home PC pe RealTraffic CPI Worker setup karne ka step-by-step process hai.**
...
### RealTraffic Account
- ✅ realtraffic.online pe **active user account** hona chahiye
...
### Step 1 — RealTraffic Update Karein
Aap ke home PC pe pehle se `realtraffic-backend` aur `realtraffic-mongo` Docker chal rahe hain. Latest CPI module pull karein:
...
.\deployment\cpi\REALTRAFFIC-CPI-SETUP.ps1
```
...
File location: `C:\realtraffic\realtraffic-cpi-worker\config.yaml`

**JWT token paste karna**:
1. realtraffic.online pe login karein
2. Browser DevTools open karein (F12)
3. Application → Local Storage → `https://realtraffic.online`
...
PowerShell me:
```powershell
cd C:\realtraffic\realtraffic-cpi-worker
...
```powershell
cd C:\realtraffic
.\deployment\cpi\REALTRAFFIC-CPI-DOCTOR.ps1
```
...
```powershell
cd C:\realtraffic
.\deployment\cpi\REALTRAFFIC-CPI-WORKER-START.bat
```
...
```
Yeh `RealTrafficCPIWorker` naam ki Windows service install karega. PC reboot pe automatic chalegi.

Service manage karne ke commands:
```powershell
nssm status RealTrafficCPIWorker      # Check status
nssm restart RealTrafficCPIWorker     # Restart
nssm stop RealTrafficCPIWorker        # Stop
```

Logs:
- `C:\realtraffic\realtraffic-cpi-worker\worker.out.log`
- `C:\realtraffic\realtraffic-cpi-worker\worker.err.log`
...
### Pehli Baar
1. realtraffic.online open karein
2. Login karein
...
- RealTraffic panel **conversion_likely** mark karega worker ne workflow complete kiya
...
- Earnings reconciliation (RealTraffic vs network panel)

### Monthly (30 min)
- Magisk modules update (phone se)
- Apple ID rotation check (iOS)
- Worker code update: `cd C:\realtraffic && git pull && nssm restart RealTrafficCPIWorker`
...
- JWT expire ho jaaye → realtraffic.online pe re-login → naya token copy → config.yaml update → worker restart

---

## ⚠️ Important Reminders

1. **Conversion numbers RealTraffic pe = ESTIMATED**. Asli payout network panel pe verify karein.
...
```powershell
Get-Content C:\realtraffic\realtraffic-cpi-worker\worker.err.log -Tail 50
```
