@echo off
REM ====================================================================
REM   RealTraffic DOCTOR — Full Stack Health Check + Auto-Fix
...
title RealTraffic - DOCTOR (Health Check + Auto-Fix)
color 0E
cls

echo.
echo  ============================================================
echo    RealTraffic - DOCTOR
echo  ============================================================
echo.
...
REM ───── 2. RealTraffic containers ─────
echo [2] RealTraffic containers status:
docker ps -a --filter "name=realtraffic-" --format "    {{.Names}}: {{.Status}}"
echo.
...
for /f "delims=" %%c in ('docker ps --format "{{.Names}}" 2^>nul') do (
    if "%%c"=="realtraffic-mongo" set "MONGO_OK=1"
    if "%%c"=="realtraffic-backend" set "BACKEND_OK=1"
)

if "!MONGO_OK!"=="0" echo     [MISSING] realtraffic-mongo not running
if "!BACKEND_OK!"=="0" echo     [MISSING] realtraffic-backend not running
...
if "!BACKEND_OK!"=="1" (
    for /f "delims=" %%e in ('docker exec realtraffic-backend env 2^>nul ^| findstr /B /C:"ADMIN_EMAIL=" /C:"ADMIN_PASSWORD=" /C:"JWT_SECRET_KEY="') do (
...
echo [7] MongoDB ping:
if "!MONGO_OK!"=="1" (
    docker exec realtraffic-mongo mongosh --quiet --eval "db.runCommand({ping:1}).ok" 2>nul
) else (
...
set "TEST_PASS="
if "!BACKEND_OK!"=="1" (
    for /f "tokens=1,* delims==" %%a in ('docker exec realtraffic-backend env 2^>nul ^| findstr /B /C:"ADMIN_EMAIL="') do set "TEST_EMAIL=%%b"
    for /f "tokens=1,* delims==" %%a in ('docker exec realtraffic-backend env 2^>nul ^| findstr /B /C:"ADMIN_PASSWORD="') do set "TEST_PASS=%%b"
...
docker ps --filter "name=realtraffic-cloudflared" --format "    {{.Names}}: {{.Status}}" 2>nul
docker ps --filter "name=realtraffic-cloudflared" --format "{{.Names}}" 2>nul | findstr "realtraffic-cloudflared" >nul
...
echo  ============================================================
echo.
echo    [1] Reset admin to admin@realtraffic.local / admin123
...
REM ════════════════════════════════════════════════════════════
:RESET_DEFAULT
set "NEW_EMAIL=admin@realtraffic.local"
...
if "!NEW_EMAIL!"=="" set "NEW_EMAIL=admin@realtraffic.local"
set "NEW_PASS="
...
if defined WORKDIR (
    ...
    docker logs --tail=100 realtraffic-backend 2>nul
)
goto END
