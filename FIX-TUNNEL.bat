...
title RealTraffic - Cloudflare Tunnel Fix
color 0B

REM ════════════════════════════════════════════════════════════
REM  RealTraffic Tunnel Fixer
...
echo.
echo  ============================================================
echo   RealTraffic Cloudflare Tunnel Fix
echo  ============================================================
echo.
echo   Folder: !ROOT!
echo.
...
if not exist "!ROOT!\.env" (
    echo  [X] .env file nahi mili. Pehle RealTraffic-AUTO.bat chala ke
...
echo   -^> realtraffic tunnel -^> Configure -^> token copy^)
echo.
set "NEW_TOKEN="
...
docker rm -f realtraffic-cloudflared >nul 2>&1
echo        OK
echo.
...
echo   Status check
echo  ============================================================
docker ps --filter "name=realtraffic-cloudflared" --format "    {{.Names}}: {{.Status}}"
echo.
echo  Cloudflared container logs ^(last 15 lines^):
docker logs --tail 15 realtraffic-cloudflared 2>&1
echo.
...
powershell -NoProfile -Command "try { $r = Invoke-RestMethod -Uri 'https://api.realtraffic.online/api/admin/login' -Method POST -Body (@{email='admin@realtraffic.local';password='admin123'} ^| ConvertTo-Json) -ContentType 'application/json' -TimeoutSec 20; if ($r.access_token) { Write-Host '  PUBLIC LOGIN: PASS  ✓' -ForegroundColor Green } else { Write-Host '  PUBLIC LOGIN: FAIL (no token)' -ForegroundColor Red } } catch { Write-Host ('  PUBLIC LOGIN: FAIL - ' + $_.Exception.Message) -ForegroundColor Red }"

echo.
echo  ============================================================
echo   Done. Ab browser mein https://realtraffic.online/admin
echo   khol ke admin@realtraffic.local / admin123 se login karo.
echo  ============================================================
echo.
pause
endlocal
