@echo off
REM =====================================================================
REM   RealTraffic ONE-CLICK AUTO (Always Fresh + GitHub Auto-Update)
REM   ───────────────────────────────────────────────────────────
...
title RealTraffic ONE-CLICK AUTO
color 0A
...
if not exist "!ROOT!\docker-compose.yml" (
    ...
    echo      Folder: !ROOT!
    echo      Yeh file RealTraffic project root mein chalayein.
    pause
    exit /b 1
)
echo  Folder: !ROOT!
...
if not exist "!ROOT!\.git" (
    ...
    echo.
    echo  Example: https://github.com/yourname/realtraffic.git
    set "GITHUB_URL="
...
set "ADMIN_EMAIL_VAL=admin@realtraffic.local"
set "ADMIN_PASS_VAL=admin123"
set "APP_URL_VAL=https://realtraffic.online"
set "PUBLIC_BASE_URL_VAL=https://api.realtraffic.online"
...
set "OLD_WORKDIR="
for /f "delims=" %%p in ('docker inspect realtraffic-backend --format "{{ index .Config.Labels \"com.docker.compose.project.working_dir\" }}" 2^>nul') do (
    set "OLD_WORKDIR=%%p"
)
if defined OLD_WORKDIR (
    if exist "!OLD_WORKDIR!\.env" (
        for /f "usebackq tokens=1,* delims==" %%a in ("!OLD_WORKDIR!\.env") do (
            if /I "%%a"=="ADMIN_EMAIL"    if not "%%b"=="" if "!ADMIN_EMAIL_VAL!"=="admin@realtraffic.local" set "ADMIN_EMAIL_VAL=%%b"
...
set "BACKUP_ROOT=F:\online\real flow\realtraffic-backups"
if not exist "F:\" set "BACKUP_ROOT=!ROOT!\..\realtraffic-backups"
...
docker exec realtraffic-backend tar czf /tmp/_uploads.tar.gz -C /app/backend uploaded_resources real_user_traffic_results 2>nul
if not errorlevel 1 (
    docker cp realtraffic-backend:/tmp/_uploads.tar.gz "!BACKUP_DIR!\volumes\_live_uploads.tar.gz" 2>nul
    if exist "!BACKUP_DIR!\volumes\_live_uploads.tar.gz" (
        echo        Backed up: live container uploads ^(first-migration safety^)
    )
    docker exec realtraffic-backend rm /tmp/_uploads.tar.gz 2>nul
)
...
docker compose down --remove-orphans 2>nul
popd
for /f "delims=" %%c in ('docker ps -a --filter "name=realtraffic-" --format "{{.Names}}" 2^>nul') do (
...
for /f "delims=" %%n in ('docker network ls --filter "name=realtraffic" --format "{{.Name}}" 2^>nul') do (
...
> "!ENV_FILE!" echo DB_NAME=realtraffic
...
>> "!ENV_FILE!" echo RESEND_FROM=no-reply@realtraffic.online
...
if not defined BACKEND_OK (
    ...
    docker logs realtraffic-backend --tail 40
    pause
    exit /b 1
)
echo.
...
>> "!SEED_SCRIPT!" echo     db = cli[os.environ.get('DB_NAME','realtraffic')]
>> "!SEED_SCRIPT!" echo     email = os.environ.get('ADMIN_EMAIL','admin@realtraffic.local').strip().lower()
...
docker cp "!SEED_SCRIPT!" realtraffic-backend:/tmp/_seed_admin.py >nul 2>&1
docker exec realtraffic-backend python /tmp/_seed_admin.py 2>&1
...
if defined LOGIN_OK (
    ...
    docker logs realtraffic-backend --tail 30
    echo  ================================================================
)
echo.
...
if defined LOGIN_OK (
    if exist "!ROOT!\.git" (
        ...
        schtasks /Delete /TN "RealTrafficAutoUpdate" /F >nul 2>&1
        REM Create scheduled task
        schtasks /Create /TN "RealTrafficAutoUpdate" /TR "\"%~f0\" _WATCH_" /SC MINUTE /MO 5 /RL HIGHEST /F >nul 2>&1
        if errorlevel 1 (
...
echo     "!ROOT!\RealTraffic-AUTO.bat" _UNINSTALL_
echo  ----------------------------------------------------------------
echo.
...
schtasks /Delete /TN "RealTrafficAutoUpdate" /F 2>&1
echo.
