@echo off
REM ============================================================================
REM   REALTRAFFIC CPI - ONE CLICK (FINAL - Android + iOS READY)
REM   ═══════════════════════════════════════════════════════════
...
title RealTraffic CPI - FINAL Setup (Android + iOS)
color 0A
cls

echo.
echo  ================================================================
echo    REALTRAFFIC CPI - FINAL SETUP (Android + iOS)
...
set "WORKER=!ROOT!\realtraffic-cpi-worker"
...
if not exist "!WORKER!" (
    echo  [FATAL] realtraffic-cpi-worker folder nahi mila.
...
powershell -NoProfile -Command ^
    "try {" ^
        ...
        "$out = @(); $found = $false;" ^
        "foreach ($l in $lines) {" ^
            "if ($l -match '^\s*backend_url\s*:') { $out += 'backend_url: \"https://api.realtraffic.online\"'; $found = $true }" ^
            "else { $out += $l }" ^
        "};" ^
        "if (-not $found) { $out += 'backend_url: \"https://api.realtraffic.online\"' };" ^
...
echo      https://realtraffic.online/admin
echo    Login (admin@realtraffic.local / admin123)
...
set "WORKER_BAT=!ROOT!\deployment\cpi\REALTRAFFIC-CPI-WORKER-START.bat"
if exist "!WORKER_BAT!" (
    echo    Starting CPI Worker in new terminal window...
    start "RealTraffic CPI Worker" cmd /k "!WORKER_BAT!"
...
echo.
echo    VERIFY:
echo      Web UI: https://realtraffic.online/admin
echo      -^> CPI Module -^> CPI Devices page
echo      -^> Connected phones "Online" dikhne chahiye
echo.
echo    MANAGE WORKER:
echo      Start : !ROOT!\deployment\cpi\REALTRAFFIC-CPI-WORKER-START.bat
echo      Stop  : !ROOT!\deployment\cpi\REALTRAFFIC-CPI-WORKER-STOP.bat
echo.
echo    Log: !LOG!
echo  ================================================================
echo.
