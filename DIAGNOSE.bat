...
(
    echo ==========================================
    echo  RealTraffic Diagnose Report
    echo  Date: %DATE% %TIME%
    echo ==========================================
    echo.
    ...
    docker ps --filter "name=realtraffic-backend" --format "{{.Names}} | {{.Status}} | {{.Image}}" 2^>^&1
    echo.

    echo === 4. Mongo container status ===
    docker ps --filter "name=realtraffic-mongo" --format "{{.Names}} | {{.Status}}" 2^>^&1
    echo.

    echo === 5. Cloudflared container status ===
    docker ps --filter "name=realtraffic-cloudflared" --format "{{.Names}} | {{.Status}}" 2^>^&1
    echo.

    echo === 6. Backend last 40 log lines ===
    docker logs --tail 40 realtraffic-backend 2^>^&1
    echo.
) >> "%OUT%"
...
powershell -NoProfile -Command ^
    ...
    "try {" ^
        "$body = @{email='admin@realtraffic.local';password='admin123'} | ConvertTo-Json;" ^
        ...
        "try {" ^
            "$u2 = Invoke-RestMethod -Uri 'https://api.realtraffic.online/api/admin/users' -Headers @{Authorization=('Bearer ' + $tok)} -TimeoutSec 20;" ^
