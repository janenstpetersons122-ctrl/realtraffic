...
title RealTraffic - Admin Credentials Manager
color 0B

REM ====================================================================
REM   RealTraffic ADMIN CREDENTIALS MANAGER (v2 - smart folder detection)
REM   Auto-detects the folder where realtraffic-backend container was created.
REM ====================================================================

cls
echo.
echo  ============================================================
echo    RealTraffic - ADMIN CREDENTIALS MANAGER
echo  ============================================================
echo.

REM ───── Smart project root detection ─────
REM Priority 1: Find folder where running realtraffic-backend container was started from
...
for /f "usebackq delims=" %%p in (`docker inspect realtraffic-backend --format "{{ index .Config.Labels \"com.docker.compose.project.working_dir\" }}" 2^>nul`) do (
    set "ROOT=%%p"
...
if "%ROOT%"=="" (
    if exist "F:\online\real flow\real flow amna\realtraffic-amna-main\realtraffic-amna-main\docker-compose.yml" (
        set "ROOT=F:\online\real flow\real flow amna\realtraffic-amna-main\realtraffic-amna-main"
...
if "%ROOT%"=="" (
    ...
    echo            F:\online\real flow\real flow amna\realtraffic-amna-main\realtraffic-amna-main\.env
    pause & exit /b 1
)
...
:ASK_EMAIL
set "NEW_EMAIL="
set /p "NEW_EMAIL=   Naya Admin Email (e.g. admin@realtraffic.local): "
if "%NEW_EMAIL%"=="" (
...
for /f "usebackq delims=" %%c in (`docker ps -a --filter "name=^realtraffic-backend$" --format "{{.Names}}" 2^>nul`) do (
    echo        Found: %%c - removing to avoid name conflict...
    docker rm -f realtraffic-backend >nul 2>&1
)
echo        OK
echo.
...
echo.
echo    Browser mein RealTraffic URL kholein - Admin Login - login karein.
